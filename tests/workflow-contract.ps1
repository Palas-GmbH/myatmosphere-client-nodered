...
Assert-Contains $publish 'dist.integrity'
Assert-Contains $publish 'PACKAGE_INTEGRITY'
Assert-Contains $publish 'openssl dgst -sha512 -binary'
Assert-Contains $publish 'gh release create'
Assert-Contains $publish '--json tagName,targetCommitish,isDraft,isPrerelease'
Assert-Contains $publish 'if [[ "$is_draft" != false || "$is_prerelease" != false ]]'
